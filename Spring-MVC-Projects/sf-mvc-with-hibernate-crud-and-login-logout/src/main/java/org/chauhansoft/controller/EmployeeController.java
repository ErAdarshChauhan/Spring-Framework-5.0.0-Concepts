...
public class EmployeeController {
	...
	public String updateEmployee(@ModelAttribute("searchEmployeeBean")Employee employee,Model model) {
		container = new ClassPathXmlApplicationContext("SpringBean.xml");
		EmployeeDao dao = container.getBean("employeeDao", EmployeeDao.class);
		boolean flag = dao.updateEmployee(employee);
		if (flag) {
			model.addAttribute("msg","Record is updated successfuly........");
			return "search";
		} else {
			model.addAttribute("msg","<h3 style='color:red;'>Ooops! .....  There is no Employees.</h3>");
			return "search";
	...
	public String deleteEmployee(@ModelAttribute("searchEmployeeBean")Employee employee,Model model) {
		container = new ClassPathXmlApplicationContext("SpringBean.xml");
		EmployeeDao dao = container.getBean("employeeDao", EmployeeDao.class);
		boolean flag = dao.deleteEmployee(employee);
		if (flag) {
			model.addAttribute("msg","Record is deleted successfuly........");
			return "search";
		} else {
			model.addAttribute("msg","<h3 style='color:red;'>Ooops! .....  There is no Employees.</h3>");
			return "search";
